(() => {
  ...
  function scoreImpactValue(source) { return scoreScenario(source).max > 0 ? `+0~${scoreScenario(source).max}점` : '제안 근거 강화'; }
  ...
  function coachAdvice() {
    const b = burden();
    const next = nextRecommendedSource();
  ...
  const stableDestinations = [
    { id:'PH', name:'필리핀', currency:'PHP', localRate:56.7, decimals:0 },
    { id:'VN', name:'베트남', currency:'VND', localRate:25400, decimals:0 },
    { id:'MN', name:'몽골', currency:'MNT', localRate:3400, decimals:0 },
    { id:'US', name:'미국', currency:'USD', localRate:1, decimals:2 },
  ];
  function stableDestination() { return stableDestinations.find(item=>item.id===state.stableDestination) || stableDestinations[0]; }
  function localReceive(usdc,destination=stableDestination()) {
    return Number(usdc) * destination.localRate;
  }
  function localReceiveText(usdc,destination=stableDestination()) {
    return `${localReceive(usdc,destination).toLocaleString('ko-KR',{minimumFractionDigits:destination.decimals,maximumFractionDigits:destination.decimals})} ${destination.currency}`;
  }
  ...
  function nextRecommendedSource() {
    const order = state.profile === 'employee' ? ['health','tax','card'] : state.profile === 'freelancer' ? ['platform','tax','card'] : ['tax','platform','bank'];
    return order.map(id=>sourceCatalog.find(s=>s.id===id)).find(s=>s&&!state.connected[s.id]) || sourceCatalog.find(s=>!state.connected[s.id]);
  }
  ...
  function homeScreen() {
    ...
    return `<section class="screen" data-screen-view="home">
      <div class="passport">
        <div class="passport-top"><div><small>MY:D DATA → MY:ME FINANCE</small><h1>내 데이터로<br>금융 기회를 키워요</h1></div><span class="demo-pill"><i></i>바로 체험</span></div>
        ...
        <div class="passport-actions"><button data-nav="connect"><span>${svg('link')}</span><b>연결</b><small>${next?`+0~${scoreScenario(next).max}점`:'준비 완료'}</small></button><button data-nav="compare"><span>${svg('gavel')}</span><b>대출</b><small>${auctionBids().length}곳 제안</small></button><button data-nav="stable"><span>${svg('coins')}</span><b>송금</b><small>${stableAuctionBids().length}곳 제안</small></button></div>
      </div>
      <div class="myd-bridge"><span>${svg('shield')}</span><div><b>My:D에서 시작한 데이터 주도권</b><p>내가 고른 데이터만 my:ME 금융 비교에 활용하고, 연결 상태와 사용 목적은 언제든 직접 관리해요.</p></div></div>
      <div class="notice safe"><b>${next?`${next.name}을 연결하면 다음 기회가 열려요`:'좋은 조건을 비교할 준비가 됐어요'}</b>${next?`예상 점수 +0~${scoreScenario(next).max}점, 조건 선택폭 +${coverageGain(next)}%p를 먼저 확인하고 연결해보세요.`:'연결한 강점으로 대출·송금 조건을 바로 비교해보세요.'}</div>
      <article class="home-stable"><div class="home-stable-head"><div class="home-coin">${svg('coins')}</div><div><span class="eyebrow">USDC PARTNER RAIL</span><h2>더 좋은 해외송금 조건을 한 번에</h2><p>여러 송금사가 총비용·현지 수령액·도착시간을 제안하면 내가 가장 마음에 드는 조건을 골라요.</p></div></div><div class="home-stable-flow"><span>원화</span><b>›</b><span>제휴사 비교</span><b>›</b><span>USDC 정산</span><b>›</b><span>현지 수령</span></div><div class="home-stable-numbers"><div><small>${destination.name} 예상 수령액</small><strong>${localReceiveText(stable.usdc,destination)}</strong><span>${stable.usdc.toFixed(2)} USDC 정산 기준</span></div><div><small>총비용 예시</small><strong>${stable.fee.toLocaleString('ko-KR')}원</strong></div></div><button class="stable-entry" data-nav="stable">지금 3개 송금사 제안 받기 ${svg('arrow')}</button></article>
  ...
  function stableScreen() {
    ...
    const steps=[
      ...
      ['2','제안 골라보기','총비용·현지 수령액·도착시간이 가장 마음에 드는 제안을 골라요.'],
    ...
    return `<section class="screen stable-screen" data-screen-view="stable">
      <article class="stable-detail-hero"><div><span class="eyebrow">MY:ME REMITTANCE</span><h1>여러 송금사가<br>더 좋은 조건을 제안해요</h1><p>국가와 금액만 고르면 총비용, 현지 수령액, 도착시간을 한 번에 비교하고 내가 직접 선택해요.</p></div><div class="stable-detail-orb"><span>${destination.currency}</span><strong>${localReceiveText(shownQuote.usdc,destination).replace(' '+destination.currency,'')}</strong><small>현지 예상 수령액 · ${shownQuote.usdc.toFixed(2)} USDC 정산 기준</small></div></article>
      <div class="partner-rail-note"><span>${svg('shield')}</span><p><b>my:ME는 조건을 비교해요.</b> 실제 환전·USDC 정산·현지 지급은 신고·인가 요건을 확인한 제휴 송금사가 진행해요.</p></div>

      <article class="remit-auction-card"><div class="remit-auction-head"><div><span class="eyebrow">REMITTANCE REVERSE AUCTION</span><h2>한 번에 받고, 가장 좋은 제안 고르기</h2><p>총비용·현지 수령액·도착시간을 같은 기준으로 바로 비교해요.</p></div><div class="remit-gavel">${svg('gavel')}</div></div><div class="remit-selects"><label><span>어디로 보낼까요?</span><select data-select="stableDestination">${stableDestinations.map(item=>`<option value="${item.id}" ${item.id===destination.id?'selected':''}>${item.name} · ${item.currency}</option>`).join('')}</select></label><label><span>어떻게 받을까요?</span><select data-select="stableReceive"><option value="bank" ${state.stableReceive==='bank'?'selected':''}>현지 은행계좌</option><option value="wallet" ${state.stableReceive==='wallet'?'selected':''}>지원 지갑</option><option value="cash" ${state.stableReceive==='cash'?'selected':''}>제휴 수령처</option></select></label></div><div class="field remit-amount"><label><span>얼마를 보낼까요?</span><b>${fmt(state.stableAmount)}만원</b></label><input data-range="stableAmount" type="range" min="10" max="3000" step="10" value="${state.stableAmount}"></div>
        ${state.stableAuctionRun?`<div class="remit-auction-summary"><div><b>${bids.length}개 제안이 도착했어요</b><span>${destination.name} · ${state.stableReceive==='bank'?'현지 은행계좌':state.stableReceive==='wallet'?'지원 지갑':'제휴 수령처'}</span></div><button data-stable-auction-run>새 제안 받기</button></div><div class="remit-bid-list">${bids.map((bid,i)=>`<button class="remit-bid ${selected.id===bid.id?'selected':''}" data-stable-bid="${bid.id}"><span class="remit-rank">${i+1}</span><div class="remit-bid-brand"><span>${bid.initial}</span><div><b>${bid.name}</b><small>${bid.tag} · USDC 기반 정산 예시</small></div></div><div class="remit-bid-metrics"><div><small>총비용</small><strong>${bid.fee.toLocaleString('ko-KR')}원</strong></div><div><small>현지 예상 수령액</small><strong>${localReceiveText(bid.usdc,destination)}</strong></div><div><small>도착시간</small><strong>${bid.speed}</strong></div></div>${selected.id===bid.id?`<span class="remit-selected">${svg('check')} 내 선택</span>`:''}</button>`).join('')}</div><button class="primary wide remit-continue" data-stable-continue>이 조건으로 다음 단계 보기 ${svg('arrow')}</button>`:`<div class="remit-auction-stage"><div><span>A</span><small>총비용</small></div><i></i><div><span>B</span><small>도착시간</small></div><i></i><div><span>C</span><small>현지 수령액</small></div></div><button class="primary wide" data-stable-auction-run>지금 3개 송금사 제안 받기 ${svg('arrow')}</button>`}
      </article>

      <article class="stable-calc-card"><div class="stable-title-row"><div><span class="eyebrow">MY CHOICE</span><h2>${state.stableAuctionRun?'내가 고른 송금 조건':'받을 금액 먼저 보기'}</h2></div><span class="sample-tag">체험 계산</span></div><div class="stable-detail-quote"><div><small>내가 내는 총비용</small><strong>${shownQuote.fee.toLocaleString('ko-KR')}원</strong><span>${state.stableAuctionRun?selected.name:'기본 0.4%'} 예시</span></div><div><small>${destination.name} 예상 수령액</small><strong>${localReceiveText(shownQuote.usdc,destination)}</strong><span>${shownQuote.usdc.toFixed(2)} USDC · 1 USDC=1,380원 정산 예시</span></div></div><div class="stable-detail-flow"><div><span>${svg('wallet')}</span><b>원화</b></div><i>›</i><div><span>${svg('shield')}</span><b>제휴 송금사</b></div><i>›</i><div><span>${svg('coins')}</span><b>USDC 정산</b></div><i>›</i><div><span>${svg('user')}</span><b>${destination.name} 현지 수령</b></div></div><small class="stable-calc-note">최종 환율·현지 통화 수령액·수수료는 선택한 제휴 송금사 화면에서 다시 확인해요.</small></article>
  ...
  function connectScreen() {
    ...
    return `<section class="screen" data-screen-view="connect"><span class="eyebrow">MY:D DATA CONNECTION</span><h1 class="screen-title">내 데이터를 연결하면<br>금융 선택지가 넓어져요</h1><p class="screen-lead">연결 전에 예상 점수 변화와 넓어지는 대출 선택폭을 먼저 확인하세요.</p>
      ...
      ${grouped.map(cat=>`<div class="category-label">${cat}</div><div class="source-list">${sourceCatalog.filter(s=>s.category===cat).map(s=>`<article class="source-card ${state.connected[s.id]?'is-connected':''}"><div class="source-icon ${s.class}">${svg(s.icon)}</div><div class="source-copy"><div class="source-title-row"><h3>${s.name}</h3>${state.connected[s.id]?`<span class="connected">${svg('check')} 반영 중</span>`:''}</div><p>${s.desc}</p></div><div class="score-impact"><span class="score-arrow">↗</span><div><small>${scoreScenario(s).max>0?'예상 점수 변화':'대출 제안 효과'}</small><strong class="${scoreScenario(s).max>0?'score-number':'score-text'}">${scoreImpactValue(s)}</strong></div><span class="coverage-chip">선택폭 +${coverageGain(s)}%p</span></div>${state.connected[s.id]?`<button class="connected-action" disabled>${svg('check')} 내 조건 비교에 반영 중</button>`:`<button class="connect-btn gain" data-source="${s.id}"><span>${s.name} 연결하기</span><strong>+${s.points} XP</strong></button>`}</article>`).join('')}</div>`).join('')}
  ...
  function compareScreen() {
    const b = burden(); const refi = refinance(); const bids = auctionBids(); const ready = coverage() >= 45; const next = nextRecommendedSource();
    return `<section class="screen" data-screen-view="compare"><span class="eyebrow">LOAN MATCH</span><h1 class="screen-title">여러 금융사가<br>내게 먼저 제안해요</h1><p class="screen-lead">희망금액을 정하고 금리·한도·월 납입액이 가장 마음에 드는 제안을 골라보세요.</p>
      <article class="data-decision-bridge"><span>${svg('link')}</span><div><small>MY:D 데이터 반영</small><b>${connectedSources().length}개 연결 · 준비도 ${coverage()}%</b><p>${connectedSources().map(s=>s.name).join(' · ')} 데이터가 제안 비교 근거에 반영돼요.</p></div>${next?`<button data-mission="source:${next.id}">${next.name}<strong>+0~${scoreScenario(next).max}점</strong></button>`:`<button data-nav="connect">연결 관리<strong>준비 완료</strong></button>`}</article>
  ...
  function modalView() {
    ...
    if (m.type === 'source') {
      ...
      const destination=stableDestination();
      content = `<div class="sheet-head"><div><span class="eyebrow">MY REMITTANCE CHOICE</span><h2>내가 고른 ${bid.name}</h2><p>${destination.name} · ${fmt(state.stableAmount)}만원 송금 조건</p></div><button class="sheet-close" data-close aria-label="닫기">${svg('close')}</button></div><div class="stable-bid-review"><div><small>내가 내는 총비용</small><strong>${bid.fee.toLocaleString('ko-KR')}원</strong></div><div><small>현지 예상 수령액</small><strong>${localReceiveText(bid.usdc,destination)}</strong></div><div><small>도착시간</small><strong>${bid.speed}</strong></div></div><div class="consent-box"><div class="consent-row"><span class="consent-check">${svg('shield')}</span><div><b>확인된 제휴사 조건</b><span>신고·인허가 요건을 계약 전에 확인해 안심할 수 있는 조건만 연결해요.</span></div></div><div class="consent-row"><span class="consent-check">${svg('user')}</span><div><b>내 정보는 필요한 만큼만</b><span>실제 이용 시 제휴사가 본인확인과 수취인 확인을 진행해요.</span></div></div><div class="consent-row"><span class="consent-check">${svg('eye')}</span><div><b>보내기 전 한 번 더 선명하게</b><span>환율, 수수료, USDC 정산 기준, 현지 통화 수령액을 최종 화면에서 다시 보여드려요.</span></div></div></div><div class="notice info"><b>지금은 개인정보 없이 선택 흐름을 체험해요</b>실제 환전·USDC 정산·현지 지급은 신고·인가 요건을 확인한 제휴 송금사가 진행해요.</div><button class="primary wide" data-stable-review>이 제안 저장하고 +15 XP</button><button class="secondary wide" style="margin-top:8px" data-close>다른 제안도 둘러보기</button>`;
  ...
  function customerGate() {
    if (gateMode==='confirm') return `<div class="customer-gate"><main class="gate-shell gate-confirm"><div class="gate-brand"><div class="brand-mark">m:M</div><strong>my:ME</strong></div><div class="gate-success">${svg('qr')}</div><span class="gate-kicker">MY:D DATA → MY:ME FINANCE</span><h1>준비됐어요<br>지금 금융 기회를 열어요</h1><p>내가 고른 데이터로 대출·송금사 제안을 한 번에 비교해보세요.</p><div class="gate-benefits"><div>${svg('spark')}<span><b>내 점수 성장 가능성</b>연결 전에 예상 변화를 보여드려요</span></div><div>${svg('gavel')}<span><b>금융사 제안 한 번에</b>금리와 한도를 보고 직접 골라요</span></div><div>${svg('coins')}<span><b>송금사 제안 한 번에</b>총비용·현지 수령액·도착시간을 비교해요</span></div></div><button class="primary wide gate-primary" data-qr-confirm>지금 my:ME 시작하기 ${svg('arrow')}</button><small class="gate-caption">개인정보 없는 고객 체험 · 실제 서비스는 정식 인증과 제휴 후 연결돼요.</small></main></div>`;
    ...
    return `<div class="customer-gate"><main class="gate-shell"><div class="gate-brand"><div class="brand-mark">m:M</div><strong>my:ME</strong><span>바로 체험</span></div><section class="gate-hero"><span class="gate-kicker">MY:D DATA → MY:ME FINANCE</span><h1>내 데이터로<br>더 좋은 조건을 만나요</h1><p>My:D에서 시작한 데이터 주도권으로 대출·송금 조건을 직접 비교해보세요.</p><div class="gate-visual"><div class="gate-score"><strong>+0~</strong><span>내 점수<br>성장 가능성</span></div><div class="gate-orbit o1">${svg('link')}<small>데이터 연결</small></div><div class="gate-orbit o2">${svg('gavel')}<small>대출 제안</small></div><div class="gate-orbit o3">${svg('coins')}<small>송금 제안</small></div></div></section><div class="gate-actions"><button class="primary wide gate-primary" data-open-qr>${svg('qr')} QR로 바로 시작하기</button><button class="secondary wide" data-guest-start>지금 이 휴대폰에서 체험하기</button></div><div class="gate-trust">${svg('shield')}<span><b>내 데이터 선택권은 언제나 나에게</b>개인정보 없이 연결 효과와 제안 비교부터 가볍게 체험해요.</span></div></main></div>`;
  }
  ...
  if ('serviceWorker' in navigator && location.protocol.startsWith('http')) {
    navigator.serviceWorker.register('./sw.js?v=14',{updateViaCache:'none'}).then(reg=>reg.update()).catch(()=>{});
  }
  render();
})();
