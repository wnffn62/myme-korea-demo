(() => {
  ...
  function homeScreen() {
    ...
    return `<section class="screen" data-screen-view="home">
      <div class="passport">
        <div class="passport-top"><div><small>MY CREDIT GROWTH MAP</small><h1>내 데이터를 연결해<br>신용을 키워요</h1></div><span class="demo-pill"><i></i>기능 체험</span></div>
  ...
  function stableScreen() {
    ...
    const uses=[
      ...
      ['store','해외 사업대금 정산','파트너 비용과 도착 예상액을 투명하게 비교해요.'],
    ];
    const steps=[
      ['1','금액 입력','원화 기준으로 보낼 금액을 정해요.'],
      ['2','본인확인','신고된 전문 사업자가 KYC를 진행해요.'],
      ['3','USDC 정산','환율과 비용을 확인한 뒤 USDC로 정산해요.'],
    ...
    return `<section class="screen stable-screen" data-screen-view="stable">
      ...
      <article class="stable-safety"><div class="stable-safety-head"><span>${svg('shield')}</span><div><span class="eyebrow">SAFE PARTNER RAIL</span><h2>안전 기준을 먼저 확인해요</h2></div></div><div class="stable-safety-list"><div>${svg('check')}<span><b>FIU 신고 사업자 연결</b>my:ME가 직접 보관·교환·전송하지 않고 확인된 파트너로 연결해요.</span></div><div>${svg('check')}<span><b>KYC·AML 확인</b>본인확인과 자금세탁방지 절차를 거쳐요.</span></div><div>${svg('check')}<span><b>트래블룰·제재주소 점검</b>송수신 정보와 위험 주소를 파트너가 확인해요.</span></div><div>${svg('check')}<span><b>비용 사전 안내</b>환율, 수수료, 예상 수령량을 확인한 뒤 선택해요.</span></div></div><button class="secondary wide" data-action="stablecoin">파트너 이용 안전장치 보기</button></article>

      <div class="notice info"><b>현재는 해외송금 이용 구조를 체험하는 화면이에요</b>실제 송금은 제휴 계약, 국가별 법률 검토, 파트너 API 연결과 본인확인 절차를 완료한 뒤 제공해요.</div><button class="primary wide stable-advisor" data-action="advisor">${svg('mic')} AI 음성상담으로 물어보기</button>
    </section>`;
  }
